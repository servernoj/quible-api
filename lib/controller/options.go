...
// Function implements optional endpoint for a service under `/email-tester` and is meant to help with email template debugging
// when it is called during controller setup phase it receives a map of handlers implemented in the controller. The `Handler`
// param in the DTO should match one of the handlers in that map and `Args` should match its arguments signature.
// This helper is meant to be used during development and is advised not to be used for prod deployment.
func WithEmailTester(Handlers map[string]any) Option {
	...
	emailTester := func(c *gin.Context) {
		...
		if _, ok := Handlers[testEmailDTO.Handler]; !ok {
			...
			c.String(http.StatusBadRequest, "invalid handler, supposed to be one of %+v", keys)
			return
		}
		fn := reflect.ValueOf(Handlers[testEmailDTO.Handler])
		if fn.Kind().String() != "func" {
			c.String(http.StatusBadRequest, "%q is a %q, not a function", testEmailDTO.Handler, fn.Kind().String())
			return
		}
		// -- args
	...
	return func(g *gin.RouterGroup) {
		g.POST("/email-tester", emailTester)
	}
}
