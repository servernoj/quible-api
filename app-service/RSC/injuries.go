...
func (client *Client) GetInjuries(query url.Values) ([]InjuryItem, error) {
	for queryKey := range query {
		client.Query.Add(queryKey, query.Get(queryKey))
	}
	req, err := http.NewRequest(
		http.MethodGet,
		fmt.Sprintf("%s/injuries/%s?%s", client.URL, client.Sport, client.Query.Encode()),
		http.NoBody,
	)
	if err != nil {
