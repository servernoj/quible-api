...
func (client *Client) GetPlayerInfo(query url.Values) ([]PlayerInfoItem, error) {
	for queryKey := range query {
		client.Query.Add(queryKey, query.Get(queryKey))
	}
	req, err := http.NewRequest(
		http.MethodGet,
		fmt.Sprintf("%s/player-info/%s?%s", client.URL, client.Sport, client.Query.Encode()),
		http.NoBody,
	)
	if err != nil {
