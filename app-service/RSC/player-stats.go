...
func (client *Client) GetPlayerStats(query url.Values) ([]PlayerSeasonStatItem, error) {
	...
	req, err := http.NewRequest(
		http.MethodGet,
		fmt.Sprintf("%s/player-stats/%s/%s?%s", client.URL, season, client.Sport, client.Query.Encode()),
		http.NoBody,
	)
	if err != nil {
	...
	if res.StatusCode == 304 {
		return []PlayerSeasonStatItem{}, nil
	}
	body := res.Body
	defer body.Close()
	var data PlayerSeasonStats
