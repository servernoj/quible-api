...
- one of the microservices is started in docker environment (docker hosted DB is targeted by migrations)
- one of the microservices is deployed to Render (external DB hosted in Render is targeted for migrations)
...
- it is generated by a migration script `cmd/migrate.sh` and should not edited unless it is a Go migration
- it has a filename containing `id` (timestamp) and `name`. The `id` portion is used to keep the track of applied migrations. The `name` portion is ignored and is used for readability

## Working with migration files

To create/apply/undo/redo migrations use the shell script `cmd/migrate.sh` and feed it with the commands followed by command's arguments (some usage examples can be found in the official `goose` [documentation](https://github.com/pressly/goose/blob/v3.16.0/README.md#usage)):
...
**Important** that shell script uses environment variable `ENV_DSN` to locate the DB instance and connect to it. It is important to **export** that environment variable prior running the script. For example:
```sh
export ENV_DSN='postgres://...'
cmd/migrate.sh status
```
...
1. In another terminal session export environment variable `ENV_DSN` (see instructions above)
...
```sh
cmd/migrate.sh create mytest sql
```
...
The migration file needs to be edited to implement the migration logic, but even without editing it can be applied and rolled back as follows:
